...
var mApp = function() {
    ...
    */
    var initTooltips = function() {
        ...
        jq('[data-toggle="m-tooltip"]').each(function() {
            var el = jq(this);
    ...
    */
    var initPopovers = function() {
        ...
        jq('[data-toggle="m-popover"]').each(function() {
            var el = jq(this);
    ...
    */
    var initScrollables = function() {
        jq('[data-scrollable="true"]').each(function(){
            var maxHeight;
            var height;
            var el = jq(this);
    ...
    */
    var initAlerts = function() {
        // init bootstrap popover
        jq('body').on('click', '[data-close=alert]', function() {
            jq(this).closest('.alert').hide();
        });
    }        
        ...
        */
        alert: function(options) {
            ...
            if (!options.container) {
                if (jq('.page-fixed-main-content').size() === 1) {
                    jq('.page-fixed-main-content').prepend(html);
                } else if ((jq('body').hasClass("page-container-bg-solid") || jq('body').hasClass("page-content-white")) && jq('.page-head').size() === 0) {
                    jq('.page-title').after(html);
                } else {
                    if (jq('.page-bar').size() > 0) {
                        jq('.page-bar').after(html);
                    } else {
                        jq('.page-breadcrumb, .breadcrumbs').after(html);
                    }
                }
            } else {
    ...
    return {
        ...
        */
        getBreakpoint: function(mode) {
            if (jq.inArray(mode, breakpoints)) {
...
(function (jq) {
	...
	// plugin setup
	jq.fn.mDatatable = function (options) {
		if (jq(this).length === 0) return;
		if (jq(this).hasClass('m-datatable--loaded')) return;
		if (jq(this).attr('id') === '') throw new Error('ID is required.');
		...
		var dt = {
			...
			init: function (options) {
				...
				jq(datatable).on('m-datatable--on-layout-updated', dt.afterRender);
			...
			layoutUpdate: function () {
				...
				jq(datatable).trigger('m-datatable--on-layout-updated', {table: jq(datatable.table).attr('id')});
			},
			...
			lockTable: function () {
				...
				var lock = {
					...
					init: function () {
						...
						lock.lockEnabled = jq.grep(options.columns, function (n, i) {
						...
						if (!dt.isLocked()) {
							...
							datatable.oriTable = jq(datatable.table).clone();
						}
					...
					enable: function () {
						var enableLock = function (tablePart) {
							...
							if (jq(tablePart).find('.m-datatable__lock').length > 0) {
								dt.log('Locked container already exist in: ', tablePart);
								return;
							}
							// check if no rows exists
							if (jq(tablePart).find('.m-datatable__row').length === 0) {
								dt.log('No row exist in: ', tablePart);
								return;
							}

							// locked div container
							var lockLeft = jq('<div/>').addClass('m-datatable__lock m-datatable__lock--left');
							var lockScroll = jq('<div/>').addClass('m-datatable__lock m-datatable__lock--scroll');
							var lockRight = jq('<div/>').addClass('m-datatable__lock m-datatable__lock--right');

							jq(tablePart).find('.m-datatable__row').each(function () {
								var rowLeft = jq('<tr/>').addClass('m-datatable__row').appendTo(lockLeft);
								var rowScroll = jq('<tr/>').addClass('m-datatable__row').appendTo(lockScroll);
								var rowRight = jq('<tr/>').addClass('m-datatable__row').appendTo(lockRight);
								jq(this).find('.m-datatable__cell').each(function () {
									var locked = jq(this).data('locked');
									if (typeof locked !== 'undefined') {
										if (typeof locked.left !== 'undefined' || locked === true) {
											// default locked to left
											jq(this).appendTo(rowLeft);
										}
										if (typeof locked.right !== 'undefined') {
											jq(this).appendTo(rowRight);
										}
									} else {
										jq(this).appendTo(rowScroll);
									}
								});
								// remove old row
								jq(this).remove();
							...
							if (lockEnabled.left.length > 0) {
								jq(datatable).addClass('m-datatable--lock');
								jq(lockLeft).appendTo(tablePart);
							}
							if (lockEnabled.left.length > 0 || lockEnabled.right.length > 0) {
								jq(lockScroll).appendTo(tablePart);
							}
							if (lockEnabled.right.length > 0) {
								jq(datatable).addClass('m-datatable--lock');
								jq(lockRight).appendTo(tablePart);
							}
						};

						jq(datatable.table).children().each(function () {
							var tablePart = this;
							if (jq(this).find('.m-datatable__lock').length === 0) {
			...
			fullRender: function () {
				if (dt.isLocked()) {
					var content = jq(datatable.oriTable).children();
					if (content.length > 0) {
						jq(datatable).removeClass('m-datatable--lock');
						jq(datatable.table).empty().html(content);
			...
			afterRender: function (e, args) {
				if (args.table === jq(datatable.table).attr('id')) {
					if (!dt.isLocked()) API.redraw();
					jq(datatable).ready(function () {
						// row even class
						jq(datatable.tableBody).find('.m-datatable__row:even').addClass('m-datatable__row--even');
						if (dt.isLocked()) API.redraw();
						jq(datatable.tableBody).css('visibility', '');
						jq(datatable).addClass('m-datatable--loaded');
						dt.scrollbar.call();
			...
			setupHover: function () {
				jq(datatable.tableBody).find('.m-datatable__cell').off('mouseenter', 'mouseleave')
					.on('mouseenter', function () {
						// normal table
						var row = jq(this).closest('.m-datatable__row').addClass('m-datatable__row--hover');
						var index = jq(row).index() + 1;

						// lock table
						jq(row).closest('.m-datatable__lock').parent().find('.m-datatable__row:nth-child(' + index + ')')
							.addClass('m-datatable__row--hover');
					})
					.on('mouseleave', function () {
						// normal table
						var row = jq(this).closest('.m-datatable__row').removeClass('m-datatable__row--hover');
						var index = jq(row).index() + 1;
			...
			adjustLockContainer: function () {
				...
				var containerWidth = jq(datatable.tableHead).width();
				var lockLeft = jq(datatable.tableHead).find('.m-datatable__lock--left').width();
				var lockRight = jq(datatable.tableHead).find('.m-datatable__lock--right').width();

				if (typeof lockLeft === 'undefined') lockLeft = 0;
				if (typeof lockRight === 'undefined') lockRight = 0;

				var lockScroll = Math.floor(containerWidth - lockLeft - lockRight);
				jq(datatable.table).find('.m-datatable__lock--scroll').css('width', lockScroll);
			...
			dragResize: function () {
				...
				jq(datatable.tableHead).find('.m-datatable__cell').mousedown(function (e) {
					start = jq(this);
					pressed = true;
					startX = e.pageX;
					startWidth = jq(this).width();
					jq(start).addClass('m-datatable__cell--resizing');

				}).mousemove(function (e) {
					if (pressed) {
						var i = jq(start).index();
						var tableBody = jq(datatable.tableBody);
						var ifLocked = jq(start).closest('.m-datatable__lock');

						if (ifLocked) {
							var lockedIndex = jq(ifLocked).index();
							tableBody = jq(datatable.tableBody).find('.m-datatable__lock').eq(lockedIndex);
						}

						jq(tableBody).find('.m-datatable__row').each(function (tri, tr) {
							jq(tr).find('.m-datatable__cell').eq(i)
								.width(startWidth + (e.pageX - startX))
								.children().width(startWidth + (e.pageX - startX));
						});

						jq(start).children().width(startWidth + (e.pageX - startX));
					}

				}).mouseup(function () {
					jq(start).removeClass('m-datatable__cell--resizing');
					pressed = false;
				});
			...
			initHeight: function () {
				if (options.layout.height && options.layout.scroll) {
					var theadHeight = jq(datatable.tableHead).find('.m-datatable__row').height();
					var tfootHeight = jq(datatable.tableFoot).find('.m-datatable__row').height();
					var bodyHeight = options.layout.height;
					if (typeof theadHeight !== 'undefined') bodyHeight -= theadHeight;
					if (typeof tfootHeight !== 'undefined') bodyHeight -= tfootHeight;
					jq(datatable.tableBody).height(bodyHeight);
				}
			},
			...
			setupBaseDOM: function () {
				...
				datatable.old = jq(datatable).clone();

				if (options.layout.height) jq(datatable).height(options.layout.height);

				// main element
				if (jq(datatable).prop('tagName') === 'TABLE') {
					// if main init element is <table>, wrap with div
					datatable.table = jq(datatable).removeClass('m-datatable').addClass('m-datatable__table');
					if (jq(datatable.table).parents('.m-datatable').length === 0) {
						datatable.wrap = jq('<div/>').addClass('m-datatable').addClass('m-datatable--' + options.layout.theme);
						datatable.table.wrap(datatable.wrap);
					}
				} else {
					// create table
					datatable.wrap = jq(datatable).addClass('m-datatable').addClass('m-datatable--' + options.layout.theme);
					datatable.table = jq('<table/>').addClass('m-datatable__table').appendTo(datatable);
				}

				if (typeof options.layout.class !== 'undefined') {
					jq(datatable.wrap).addClass(options.layout.class);
				}

				jq(datatable.table).removeClass('m-datatable--destroyed')
					.css('display', 'block').attr('id', mUtil.getUniqueID('m-datatable--'));

				// predefine table height
				if (options.layout.height) jq(datatable.table).height(options.layout.height);

				// for normal table load
				if (options.data.type === null) {
					jq(datatable.table).css('width', '').css('display', '');
				}

				// create table head element
				datatable.tableHead = jq(datatable.table).find('thead');
				if (jq(datatable.tableHead).length === 0) {
					datatable.tableHead = jq('<thead/>').prependTo(datatable.table);
				}

				// create table head element
				datatable.tableBody = jq(datatable.table).find('tbody');
				if (jq(datatable.tableBody).length === 0) {
					datatable.tableBody = jq('<tbody/>').appendTo(datatable.table);
				}

				if (typeof options.layout.footer !== 'undefined' && options.layout.footer) {
					// create table foot element
					datatable.tableFoot = jq(datatable.table).find('tfoot');
					if (jq(datatable.tableFoot).length === 0) {
						datatable.tableFoot = jq('<tfoot/>').appendTo(datatable.table);
					}
				}
			},
			...
			setupCellField: function (tableParts) {
				if (typeof tableParts === 'undefined') tableParts = jq(datatable.table).children();
				var columns = options.columns;
				jq.each(tableParts, function (part, tablePart) {
					jq(tablePart).find('.m-datatable__row').each(function (tri, tr) {
						// prepare data
						jq(tr).find('.m-datatable__cell').each(function (tdi, td) {
							if (typeof columns[tdi] !== 'undefined') {
								jq(td).data(columns[tdi]);
							}
						});
					});
				});
				...
				jq(tablePart).find('.m-datatable__row').each(function (tri, tr) {
					...
					if (typeof obj === 'undefined') {
						...
						jq(tr).find('.m-datatable__cell').each(function (tdi, td) {
							...
							var column = jq.grep(columns, function (n, i) {
								return jq(td).data('field') === n.field;
							})[0];
							if (typeof column !== 'undefined') {
								obj[column['field']] = jq(td).text();
							}
						});
					}
					...
					jq(tr).find('.m-datatable__cell').each(function (tdi, td) {
						...
						if (typeof column !== 'undefined') {
							...
							if (typeof column.template !== 'undefined') {
								...
								var span = jq('<span/>').append(finalValue);
								// insert to cell, wrap with span
								jq(td).html(span);

								// set span overflow
								if (typeof column.overflow !== 'undefined') {
									jq(span).css('overflow', column.overflow);
								}
							}
						}
					});
			...
			setupSystemColumn: function () {
				...
				var columns = options.columns;
				jq(datatable.tableBody).find('.m-datatable__row').each(function (tri, tr) {
					jq(tr).find('.m-datatable__cell').each(function (tdi, td) {
						// get column settings by field
						var column = jq.grep(columns, function (n, i) {
							return jq(td).data('field') === n.field;
						...
						if (typeof column !== 'undefined') {
							var value = jq(td).text();

							// enable column selector
							if (typeof column.selector !== 'undefined' && column.selector !== false) {
								// check if checkbox exist
								if (jq(td).find('.m-checkbox [type="checkbox"]').length > 0) return;
								jq(td).addClass('m-datatable__cell--check');
								// append checkbox
								var chk = jq('<label/>').addClass('m-checkbox m-checkbox--single')
									.append(jq('<input/>')
										.attr('type', 'checkbox')
										.attr('value', value)
										.on('click', function () {
								...
								if (typeof column.selector.class !== 'undefined') {
									jq(chk).addClass(column.selector.class);
								}

								jq(td).children().html(chk);
							}

							// enable column subtable toggle
							if (typeof column.subtable !== 'undefined' && column.subtable) {
								// check if subtable toggle exist
								if (jq(td).find('.m-datatable__toggle-subtable').length > 0) return;
								// append subtable toggle
								jq(td).children().html(jq('<a/>')
									.addClass('m-datatable__toggle-subtable')
									.attr('href', '#')
									.attr('data-value', value)
									.append(jq('<i/>').addClass(API.getOption('layout.icons.rowDetail.collapse'))));
							}
						}
					});
				});
				...
				var initCheckbox = function (td) {
					...
					if (typeof column !== 'undefined') {
						...
						if (typeof column.selector !== 'undefined' && column.selector !== false) {
							...
							// append checkbox
							var chk = jq('<label/>').addClass('m-checkbox m-checkbox--single m-checkbox--all')
								.append(jq('<input/>')
									.attr('type', 'checkbox')
									.on('click', function () {
										if (jq(this).is(':checked')) {
				...
				if (typeof options.layout.header !== 'undefined' && options.layout.header === true) {
					initCheckbox(jq(datatable.tableHead).find('.m-datatable__row').first().find('.m-datatable__cell').first());
				}
				if (typeof options.layout.footer !== 'undefined' && options.layout.footer === true) {
					initCheckbox(jq(datatable.tableFoot).find('.m-datatable__row').first().find('.m-datatable__cell').first());
				}
			},
			...
			adjustCellsWidth: function () {
				...
				if (colsPerRow > 0) {
					...
					jq(datatable.table).find('.m-datatable__row')
						.find('.m-datatable__cell').each(function (tdi, td) {
						var width = minWidth;
						var dataWidth = jq(td).data('width');
						if (typeof dataWidth !== 'undefined') {
							width = dataWidth;
						}
						jq(td).children().css('width', width);
					});
				}
			},
			...
			adjustCellsHeight: function () {
				jq(datatable.table).find('.m-datatable__row');
				jq.each(jq(datatable.table).children(), function (part, tablePart) {
					for (var i = 1; i <= dt.getTotalRows(tablePart); i++) {
						var rows = dt.getOneRow(tablePart, i, false);
						if (jq(rows).length > 0) {
							var maxHeight = Math.max.apply(null, jq(rows).map(function () {
								return jq(this).height();
							}).get());
							jq(rows).css('height', Math.ceil(maxHeight));
						}
					}
				});
			},
			...
			setupDOM: function (table) {
				// set table classes
				jq(table).find('> thead').addClass('m-datatable__head');
				jq(table).find('> tbody').addClass('m-datatable__body');
				jq(table).find('> tfoot').addClass('m-datatable__foot');
				jq(table).find('tr').addClass('m-datatable__row');
				jq(table).find('tr > th, tr > td').addClass('m-datatable__cell');
				jq(table).find('tr > th, tr > td').each(function (i, td) {
					if (jq(td).find('span').length === 0) {
						jq(td).wrapInner(jq('<span/>').width(dt.offset));
					}
				});
			},
				...
				var scroll = {
					...
					init: function () {
						...
						if (options.layout.scroll) {
							...
							jq(datatable).addClass('m-datatable--scroll');

							var scrollable = jq(datatable.tableBody).find('.m-datatable__lock--scroll');
							if (jq(scrollable).length > 0) {
								scroll.scrollHead = jq(datatable.tableHead).find('> .m-datatable__lock--scroll > .m-datatable__row');
								scroll.scrollFoot = jq(datatable.tableFoot).find('> .m-datatable__lock--scroll > .m-datatable__row');
								scroll.tableLocked = jq(datatable.tableBody).find('.m-datatable__lock:not(.m-datatable__lock--scroll)');
								...
								scroll.scrollHead = jq(datatable.tableHead).find('> .m-datatable__row');
								scroll.scrollFoot = jq(datatable.tableFoot).find('> .m-datatable__row');
								if (screen > mUtil.getBreakpoint('lg')) {
							...
							jq(datatable.table).css('height', 'auto').css('overflow-x', 'auto');
						}
					},
					...
					onScrolling: function (e) {
						var left = jq(this).scrollLeft();
						var top = jq(this).scrollTop();
						jq(scroll.scrollHead).css('left', -left);
						jq(scroll.scrollFoot).css('left', -left);
						jq(scroll.tableLocked).each(function (i, table) {
							jq(table).css('top', -top);
						});
					},
					mCustomScrollbar: function (scrollable) {
						...
						var mcsOptions = jq.extend({}, scroll.mcsOptions, {
							axis: axis,
							setHeight: jq(datatable.tableBody).height(),
							callbacks: {
								whileScrolling: function () {
									var mcs = this.mcs;
									jq(scroll.scrollHead).css('left', mcs.left);
									jq(scroll.scrollFoot).css('left', mcs.left);
									jq(scroll.tableLocked).each(function (i, table) {
										jq(table).css('top', mcs.top);
									});
								}
							}
						});
						...
						if (API.getOption('layout.smoothScroll.scrollbarShown') === true) {
							jq(scrollable).attr('data-scrollbar-shown', 'true');
						}

						// create a new instance for table body with scrollbar
						dt.mCustomScrollbar(scrollable, mcsOptions);
						jq(scrollable).mCustomScrollbar('scrollTo', 'top');
					}
				};
				scroll.init();
				return scroll;
			...
			mCustomScrollbar: function (element, options) {
				jq(datatable.tableBody).css('overflow', '');
				// check if any custom scrollbar exist in the element
				if (jq(element).find('.mCustomScrollbar').length === 0) {
					if (jq(datatable.tableBody).hasClass('mCustomScrollbar')) {
						jq(datatable.tableBody).mCustomScrollbar('destroy');
					}
					jq(element).mCustomScrollbar(options);
				}
			},
			...
			setHeadTitle: function (tablePart) {
				...
				var columns = options.columns;
				var row = jq(tablePart).find('.m-datatable__row');
				var ths = jq(tablePart).find('.m-datatable__cell');
				if (jq(row).length === 0) {
					row = jq('<tr/>').appendTo(tablePart);
				}
				jq.each(columns, function (i, column) {
					var th = jq(ths).eq(i);
					if (jq(th).length === 0) {
						th = jq('<th/>').appendTo(row);
					}
					// set column title
					if (typeof column['title'] !== 'undefined') {
						jq(th).html(column['title']).attr('data-field', column.field).data(column);
					}
					// apply text align to thead/tfoot
					if (typeof column.textAlign !== 'undefined') {
						var align = typeof datatable.textAlign[column.textAlign] !== 'undefined' ? datatable.textAlign[column.textAlign] : '';
						jq(th).addClass(align);
					}
				});
			 ...
			 */
			dataRender: function () {
				jq(datatable.table).siblings('.m-datatable__pager').removeClass('m-datatable--paging-loaded');

				var afterGetData = function (result) {
					jq(datatable).removeClass('m-datatable--error');
					// pagination enabled
					if (options.pagination) {
						...
						if (typeof result !== 'undefined' && options.data.serverPaging && options.data.type !== 'local') {
							...
							dt.paging(null, function (ctx, meta) {
								jq(ctx.pager).off().on('m-datatable--on-update-perpage', function (e, meta) {
									jq(ctx.pager).remove();
									ctx.init(meta);
								});
								...
								dt.updateRawData();
								datatable.jsonData = jq(datatable.jsonData).slice(start, end);
			...
			insertData: function () {
				...
				var tableBody = jq('<tbody/>').addClass('m-datatable__body').css('visibility', 'hidden');
				jq.each(datatable.jsonData, function (i, row) {
					// keep data object to row
					var tr = jq('<tr/>').attr('data-row', i).data('obj', row);
					jq.each(options.columns, function (x, column) {
						var td = jq('<td/>').append(row[column.field])
						...
						if (params.sort.field === column.field) {
							jq(td).addClass('m-datatable__cell--sorted');
						}

						// apply text align
						if (typeof column.textAlign !== 'undefined') {
							var align = typeof datatable.textAlign[column.textAlign] !== 'undefined' ? datatable.textAlign[column.textAlign] : '';
							jq(td).addClass(align);
						}
					});
					jq(tr).appendTo(tableBody);
				});

				// display no records message
				if (datatable.jsonData.length === 0) {
					jq('<span/>').addClass('m-datatable--error').width('100%').html(API.getOption('translate.records.noRecords')).appendTo(tableBody);
					jq(datatable).addClass('m-datatable--error');
				}

				// replace existing table body
				jq(datatable.tableBody).replaceWith(tableBody);
			...
			updateTableComponents: function () {
				datatable.tableHead = jq(datatable.table).children('thead');
				datatable.tableBody = jq(datatable.table).children('tbody');
				datatable.tableFoot = jq(datatable.table).children('tfoot');
			},
			...
			getData: function () {
				...
				if (options.data.type === 'remote') {
					...
					params.data['datatable'] = jq.extend({}, API.getDataSourceParam(), API.getOption('data.source.read.params'));
					params.method = 'POST';
				}
				...
				return jq.ajax(params)
					.done(function (data, textStatus, jqXHR) {
						...
						jq(datatable).trigger('m-datatable--on-ajax-done', datatable.jsonData);
					})
					.fail(function (jqXHR, textStatus, errorThrown) {
						jq(datatable).trigger('m-datatable--on-ajax-fail', [jqXHR]);
						jq('<span/>').addClass('m-datatable--error').width('100%').html(API.getOption('translate.records.noRecords')).appendTo(datatable.tableBody);
						jq(datatable).addClass('m-datatable--error');
					})
					.always(function () {
					});
			...
			paging: function (meta, callback) {
				var pg = {
					...
					init: function (meta) {
						...
						pg.pager = jq(datatable.table).siblings('.m-datatable__pager');
						if (jq(pg.pager).hasClass('m-datatable--paging-loaded')) return;
						...
						jq(datatable).trigger(pg.paginateEvent, pg.meta);
						if (pg.initCallback) {
						...
						jq(window).resize(pg.pagingBreakpoint);
					},
					...
					populate: function () {
						...
						// pager root element
						pg.pager = jq('<div/>').addClass('m-datatable__pager m-datatable--paging-loaded clearfix');
						// numbering links
						var pagerNumber = jq('<ul/>').addClass('m-datatable__pager-nav');
						pg.pagerLayout['pagination'] = pagerNumber;

						// pager first/previous button
						jq('<li/>')
							.append(jq('<a/>').attr('title', title.first)
								.addClass('m-datatable__pager-link m-datatable__pager-link--first')
								.append(jq('<i/>').addClass(icons.first))
								.on('click', pg.gotoMorePage)
								.attr('data-page', 1))
							.appendTo(pagerNumber);
						jq('<li/>')
							.append(jq('<a/>').attr('title', title.prev)
								.addClass('m-datatable__pager-link m-datatable__pager-link--prev')
								.append(jq('<i/>').addClass(icons.prev))
								.on('click', pg.gotoMorePage))
							.appendTo(pagerNumber);

						// more previous pages
						jq('<li/>').append(jq('<a/>').attr('title', title.more)
							.addClass('m-datatable__pager-link m-datatable__pager-link--more-prev')
							.html(jq('<i/>').addClass(icons.more))
							.on('click', pg.gotoMorePage))
							.appendTo(pagerNumber);

						jq('<li/>').append(jq('<input/>').attr('type', 'text')
							.addClass('m-pager-input form-control')
							.attr('title', title.input)
							.on('keyup', function () {
								// on keyup update [data-page]
								jq(this).attr('data-page', Math.abs(jq(this).val()));
							})
						...
						for (var page = 0; page < pg.meta.pages; page++) {
							...
							jq('<li/>').append(jq('<a/>').addClass('m-datatable__pager-link m-datatable__pager-link-number')
						...
						jq('<li/>')
							.append(jq('<a/>').attr('title', title.next)
								.addClass('m-datatable__pager-link m-datatable__pager-link--next')
								.append(jq('<i/>').addClass(icons.next))
								.on('click', pg.gotoMorePage))
							.appendTo(pagerNumber);
						jq('<li/>')
							.append(jq('<a/>').attr('title', title.last)
								.addClass('m-datatable__pager-link m-datatable__pager-link--last')
								.append(jq('<i/>').addClass(icons.last))
								.on('click', pg.gotoMorePage)
						...
						if (API.getOption('toolbar.items.info')) {
							pg.pagerLayout['info'] = jq('<div/>')
								.addClass('m-datatable__pager-info')
								.append(jq('<span/>').addClass('m-datatable__pager-detail'));
						}

						jq.each(API.getOption('toolbar.layout'), function (i, layout) {
							jq(pg.pagerLayout[layout]).appendTo(pg.pager);
						});

						// page size select
						var pageSizeSelect = jq('<select/>').addClass('selectpicker m-datatable__pager-size')
							.attr('title', API.getOption('translate.toolbar.pagination.items.default.select'))
							.attr('data-width', '70px')
							.val(pg.meta.perpage)
							.on('change', pg.updatePerpage)
							.prependTo(pg.pagerLayout['info']);
						jq.each(API.getOption('toolbar.items.pagination.pageSizeSelect'), function (i, size) {
							var display = size;
							if (size === -1) display = 'All';
							jq('<option/>').attr('value', size)
								.html(display)
					...
					paste: function () {
						...
						jq.each(jq.unique(API.getOption('toolbar.placement')), function (i, position) {
							if (position === 'bottom') {
								jq(pg.pager).clone(true).insertAfter(datatable.table);
							}
							if (position === 'top') {
								// pager top need some extra space
								jq(pg.pager).clone(true).addClass('m-datatable__pager--top').insertBefore(datatable.table);
							}
						});
					},
					gotoMorePage: function (e) {
						e.preventDefault();
						// jq(this) is a link of .m-datatable__pager-link

						if (jq(this).attr('disabled') === 'disabled') return false;

						var page = jq(this).attr('data-page');

						// event from text input
						if (typeof page === 'undefined') {
							page = jq(e.target).attr('data-page');
						}

						jq(pg.pager).find('.m-datatable__pager-link-number[data-page="' + page + '"]').trigger('click');
					...
					gotoPage: function (e) {
						...
						if (jq(this).hasClass('m-datatable__pager-link--active')) return;

						// currentPage is 1-based index
						pg.meta.page = parseInt(jq(this).data('page'));

						jq(datatable).trigger(pg.paginateEvent, pg.meta);
						pg.callback(pg, pg.meta);

						// update page callback function
						jq(pg.pager).trigger('m-datatable--on-goto-page', pg.meta);
					},
					updatePerpage: function (e) {
						e.preventDefault();
						if (API.getOption('layout.height') === null) {
							// fix white space, when perpage is set from many records to less records
							jq('html, body').animate({scrollTop: jq(datatable).position().top});
						}
						...
						if (e.originalEvent) {
							pg.meta.perpage = parseInt(jq(this).val());
						}

						jq(pg.pager).find('select.m-datatable__pager-size').val(pg.meta.perpage).attr('data-selected', pg.meta.perpage);

						// update datasource params
						API.setDataSourceParam('pagination', pg.meta);

						// update page callback function
						jq(pg.pager).trigger('m-datatable--on-update-perpage', pg.meta);
						jq(datatable).trigger(pg.paginateEvent, pg.meta);
						pg.callback(pg, pg.meta);
					...
					addPaginateEvent: function (e) {
						// pagination event
						jq(datatable).off(pg.paginateEvent).on(pg.paginateEvent, function (e, meta) {
							dt.spinnerCallback(true);

							pg.pager = jq(datatable.table).siblings('.m-datatable__pager');
							var pagerNumber = jq(pg.pager).find('.m-datatable__pager-nav');

							// set sync active page class
							jq(pagerNumber).find('.m-datatable__pager-link--active').removeClass('m-datatable__pager-link--active');
							jq(pagerNumber).find('.m-datatable__pager-link-number[data-page="' + meta.page + '"]')
								.addClass('m-datatable__pager-link--active');

							// set next and previous link page number
							jq(pagerNumber).find('.m-datatable__pager-link--prev').attr('data-page', Math.max(meta.page - 1, 1));
							jq(pagerNumber).find('.m-datatable__pager-link--next').attr('data-page', Math.min(meta.page + 1, meta.pages));

							// current page input value sync
							jq(pg.pager).each(function () {
								jq(this).find('.m-pager-input[type="text"]').prop('value', meta.page);
							});

							// update datasource params
							API.setDataSourceParam('pagination', pg.meta);

							jq(pg.pager).find('select.m-datatable__pager-size').val(meta.perpage).attr('data-selected', meta.perpage);

							// clear active rows
							jq(datatable.table).find('.m-checkbox > [type="checkbox"]').prop('checked', false);
							jq(datatable.table).find('.m-datatable__row--active').removeClass('m-datatable__row--active');
					...
					updateInfo: function () {
						...
						// page info update
						jq(pg.pager).find('.m-datatable__pager-info').find('.m-datatable__pager-detail')
					...
					pagingBreakpoint: function () {
						...
						var pagerNumber = jq(datatable.table).siblings('.m-datatable__pager').find('.m-datatable__pager-nav');
						if (jq(pagerNumber).length === 0) return;

						var currentPage = API.getCurrentPage();
						var pagerInput = jq(pagerNumber).find('.m-pager-input').closest('li');

						// reset
						jq(pagerNumber).find('li').show();

						// pagination update
						jq.each(API.getOption('toolbar.items.pagination.pages'), function (mode, option) {
							if (mUtil.isInResponsiveRange(mode)) {
								switch (mode) {
									...
									case 'tablet':
										...
										jq(pagerNumber).each(function (i, pager) {
											jq(pager).find('.m-datatable__pager-link-number').closest('li').hide()
									...
									case 'mobile':
										jq(pagerInput).show();
										jq(pagerNumber).find('.m-datatable__pager-link--more-prev').closest('li').hide();
										jq(pagerNumber).find('.m-datatable__pager-link--more-next').closest('li').hide();
										jq(pagerNumber).find('.m-datatable__pager-link-number').closest('li').hide();
										break;
								}
					...
					paginationUpdate: function () {
						var pager = jq(datatable.table).siblings('.m-datatable__pager').find('.m-datatable__pager-nav'),
							pagerMorePrev = jq(pager).find('.m-datatable__pager-link--more-prev'),
							pagerMoreNext = jq(pager).find('.m-datatable__pager-link--more-next'),
							pagerFirst = jq(pager).find('.m-datatable__pager-link--first'),
							pagerPrev = jq(pager).find('.m-datatable__pager-link--prev'),
							pagerNext = jq(pager).find('.m-datatable__pager-link--next'),
							pagerLast = jq(pager).find('.m-datatable__pager-link--last');

						// get visible page
						var pagerNumber = jq(pager).find('.m-datatable__pager-link-number:visible');
						// get page before of first visible
						var morePrevPage = Math.max(jq(pagerNumber).first().data('page') - 1, 1);
						jq(pagerMorePrev).each(function (i, prev) {
							jq(prev).attr('data-page', morePrevPage);
						});
						// show/hide <li>
						if (morePrevPage === 1) {
							jq(pagerMorePrev).parent().hide();
						} else {
							jq(pagerMorePrev).parent().show();
						}

						// get page after of last visible
						var moreNextPage = Math.min(jq(pagerNumber).last().data('page') + 1, pg.meta.pages);
						jq(pagerMoreNext).each(function (i, prev) {
							jq(pagerMoreNext).attr('data-page', moreNextPage).show();
						});

						// show/hide <li>
						if (moreNextPage === pg.meta.pages
							// missing dot fix when last hidden page is one left
							&& moreNextPage === jq(pagerNumber).last().data('page')) {
							jq(pagerMoreNext).parent().hide();
						} else {
							jq(pagerMoreNext).parent().show();
						}

						// begin/end of pages
						if (pg.meta.page === 1) {
							jq(pagerFirst).attr('disabled', true).addClass('m-datatable__pager-link--disabled');
							jq(pagerPrev).attr('disabled', true).addClass('m-datatable__pager-link--disabled');
						} else {
							jq(pagerFirst).removeAttr('disabled').removeClass('m-datatable__pager-link--disabled');
							jq(pagerPrev).removeAttr('disabled').removeClass('m-datatable__pager-link--disabled');
						}
						if (pg.meta.page === pg.meta.pages) {
							jq(pagerNext).attr('disabled', true).addClass('m-datatable__pager-link--disabled');
							jq(pagerLast).attr('disabled', true).addClass('m-datatable__pager-link--disabled');
						} else {
							jq(pagerNext).removeAttr('disabled').removeClass('m-datatable__pager-link--disabled');
							jq(pagerLast).removeAttr('disabled').removeClass('m-datatable__pager-link--disabled');
						}

						// display more buttons
						var nav = API.getOption('toolbar.items.pagination.navigation');
						if (!nav.first) jq(pagerFirst).remove();
						if (!nav.prev) jq(pagerPrev).remove();
						if (!nav.next) jq(pagerNext).remove();
						if (!nav.last) jq(pagerLast).remove();
					}
				};
				pg.init(meta);
				return pg;
			...
			columnHide: function () {
				...
				jq.each(options.columns, function (i, column) {
					if (typeof column.responsive !== 'undefined') {
						var field = column.field;
						var tds = jq.grep(jq(datatable.table).find('.m-datatable__cell'), function (n, i) {
							return field === jq(n).data('field');
						});
			...
			setupSubDatatable: function () {
				...
				if (typeof detailCallback === 'function') {
					...
					if (jq(datatable.table).find('.m-datatable__detail').length > 0) return;

					jq(datatable).addClass('m-datatable--subtable');
					...
					var toggleSubTable = function (e) {
						...
						var parentRow = jq(this).closest('.m-datatable__row');
						// get detail row for sub table
						var detailRow = jq(parentRow).next().toggle();
						// get id from first column of parent row
						var primaryKey = jq(this).closest('[data-field]:first-child')
							.find('.m-datatable__toggle-subtable').data('value');

						var icon = jq(this).find('i').removeAttr('class');
						if (jq(detailRow).is(':hidden')) {
							jq(icon).addClass(API.getOption('layout.icons.rowDetail.collapse'));
							// remove expand class from parent row
							jq(parentRow).removeClass('m-datatable__row--detail-expanded');
							// trigger event on collapse
							jq(datatable).trigger('m-datatable--on-collapse-detail', [parentRow]);
						} else {
							// expand and run callback function
							jq(icon).addClass(API.getOption('layout.icons.rowDetail.expand'));
							// add expand class to parent row
							jq(parentRow).addClass('m-datatable__row--detail-expanded');
							// trigger event on expand
							jq(datatable).trigger('m-datatable--on-expand-detail', [parentRow]);

							// get data from ajax
							e.data = jq.grep(datatable.jsonData, function (n, i) {
								return primaryKey === n[options.columns[0].field];
							})[0];

							// pass placeholder cell for sub table
							e.detailCell = jq(detailRow).find('.m-datatable__detail');
							if (jq(e.detailCell).find('.m-datatable').length === 0) {
					...
					jq(datatable.tableBody).find('.m-datatable__row').each(function () {
						var detailRow = jq('<tr/>').addClass('m-datatable__row-detail').hide()
							.append(jq('<td/>').addClass('m-datatable__detail')
								.attr('colspan', dt.getTotalColumns()));
						jq(this).after(detailRow);
						// add class to even row
						if (jq(this).hasClass('m-datatable__row--even')) {
							jq(detailRow).addClass('m-datatable__row-detail--even');
						}
					});
				}
			},
			 ...
			 */
			isLocked: function () {
				return jq(datatable).hasClass('m-datatable--lock') || false;
			},
			...
			replaceTableContent: function (html, tablePart) {
				if (typeof tablePart === 'undefined') tablePart = datatable.tableBody;
				if (jq(tablePart).hasClass('mCustomScrollbar')) {
					jq(tablePart).find('.mCSB_container').html(html);
				} else {
					jq(tablePart).html(html);
				}
			},
			...
			getExtraSpace: function (element) {
				var padding = parseInt(jq(element).css('paddingRight')) + parseInt(jq(element).css('paddingLeft'));
				var margin = parseInt(jq(element).css('marginRight')) + parseInt(jq(element).css('marginLeft'));
				var border = Math.ceil(jq(element).css('border-right-width').replace('px', ''));
			...
			getTableId: function (suffix) {
				...
				return jq(datatable).attr('id') + suffix;
			},
			...
			getDepth: function () {
				...
				do {
					table = jq(table).parents('.m-datatable__table');
					depth++;
				} while (jq(table).length > 0);
				return depth;
			},
			...
			stateUpdate: function (key, value) {
				...
				dt.stateKeep(key, jq.extend({}, ori, value));
			},
			...
			getTotalColumns: function (tablePart) {
				if (typeof tablePart === 'undefined') tablePart = datatable.tableBody;
				return jq(tablePart).find('.m-datatable__row').first().find('.m-datatable__cell').length;
			},
			...
			getTotalRows: function (tablePart) {
				if (typeof tablePart === 'undefined') tablePart = datatable.tableBody;
				return jq(tablePart).find('.m-datatable__row').first().parent().find('.m-datatable__row').length;
			},
			...
			getOneRow: function (tablePart, row, tdOnly) {
				...
				// get list of <tr>
				var result = jq(tablePart).find('.m-datatable__row:not(.m-datatable__row-detail):nth-child(' + row + ')');
				if (tdOnly) {
			...
			hasOverflowCells: function (element) {
				var children = jq(element).find('tr:first-child').find('.m-datatable__cell');
				var maxWidth = 0;

				if (children.length > 0) {
					jq(children).each(function (tdi, td) {
						maxWidth += Math.ceil(jq(td).innerWidth());
					});

					return maxWidth >= jq(element).outerWidth();
				}
			...
			hasOverflowX: function (element) {
				var children = jq(element).find('*');

				if (children.length > 0) {
					var maxWidth = Math.max.apply(null, jq(children).map(function () {
						return jq(this).outerWidth(true);
					}).get());

					return maxWidth > jq(element).width();
				}
			...
			hasOverflowY: function (element) {
				var children = jq(element).find('.m-datatable__row');
				var maxHeight = 0;

				if (children.length > 0) {
					jq(children).each(function (tdi, td) {
						maxHeight += Math.floor(jq(td).innerHeight());
					});

					return maxHeight > jq(element).innerHeight();
				}
			...
			sortColumn: function (header, sort, int) {
				...
				var column = jq(header).index();
				var rows = jq(datatable.tableBody).find('.m-datatable__row');
				var hIndex = jq(header).closest('.m-datatable__lock').index();
				if (hIndex !== -1) {
					rows = jq(datatable.tableBody).find('.m-datatable__lock:nth-child(' + (hIndex + 1) + ')').find('.m-datatable__row');
				}

				var container = jq(rows).parent();
				jq(rows).sort(function (a, b) {
					var tda = jq(a).find('td:nth-child(' + column + ')').text();
					var tdb = jq(b).find('td:nth-child(' + column + ')').text();
			...
			sorting: function () {
				var sortObj = {
					init: function () {
						if (options.sortable) {
							jq(datatable.tableHead).find('.m-datatable__cell').addClass('m-datatable__cell--sort').off('click').on('click', sortObj.sortClick);
							// first init
							sortObj.setIcon();
						}
					},
					setIcon: function () {
						...
						var td = jq(datatable.tableHead).find('.m-datatable__cell[data-field="' + meta.field + '"]').attr('data-sort', meta.sort);
						var sorting = jq(td).find('span');
						var icon = jq(sorting).find('i');

						var icons = API.getOption('layout.icons.sort');
						// update sort icon; desc & asc
						if (jq(icon).length > 0) {
							jq(icon).removeAttr('class').addClass(icons[meta.sort]);
						} else {
							jq(sorting).append(jq('<i/>').addClass(icons[meta.sort]));
						}
					},
					sortClick: function (e) {
						...
						jq(datatable.tableHead).find('.m-datatable__cell > span > i').remove();
						...
						if (options.sortable) {
							...
							setTimeout(function () {
								dt.dataRender();
								jq(datatable).trigger('m-datatable--on-sort', meta);
							}, 300);
						}
					}
				};
			...
			updateRawData: function () {
				var params = API.getDataSourceParam();
				if (jq.type(datatable.fullJsonData) === 'undefined') {
					datatable.fullJsonData = datatable.jsonData;
				}

				datatable.jsonData = jq(datatable.fullJsonData).sort(function (a, b) {
				...
				if (jq.type(params.query.generalSearch) === 'string') {
					datatable.jsonData = jq.grep(datatable.jsonData, function (obj) {
						for (var field in obj) {
							if (!obj.hasOwnProperty(field)) continue;
							if (jq.type(obj[field]) === 'string') {
				...
				if (jq.type(params.query) === 'object') {
					// remove empty element from array
					jq.each(params.query, function (k, v) {
						if (v === '') {
			...
			filterArray: function (list, args, operator) {
				if (jq.type(operator) === 'undefined') operator = 'AND';

				if (jq.type(args) !== 'object') {
					return list;
				}

				operator = operator.toUpperCase();

				if (jq.inArray(operator, ['AND', 'OR', 'NOT']) === -1) {
					return [];
				}
				...
				jq.each(list, function (key, obj) {
					var to_match = obj;

					var matched = 0;
					jq.each(args, function (m_key, m_value) {
			...
			resetScroll: function () {
				if (typeof options.detail === 'undefined' && dt.getDepth() === 1) {
					jq(datatable.table).find('.m-datatable__row').css('left', 0);
					jq(datatable.table).find('.m-datatable__lock').css('top', 0);
					jq(datatable.tableBody).scrollTop(0);
				}
			},
			...
			getDefaultSortColumn: function () {
				...
				jq.each(options.columns, function (i, column) {
					if (typeof column.sortable !== 'undefined'
						&& jq.inArray(column.sortable, ['asc', 'desc']) !== -1) {
			...
			getHiddenDimensions: function (element, includeMargin) {
				var props = {position: 'absolute', visibility: 'hidden', display: 'block'},
					...
					hiddenParents = jq(element).parents().addBack().not(':visible');
				...
				dim.width = jq(element).width();
				dim.outerWidth = jq(element).outerWidth(includeMargin);
				dim.innerWidth = jq(element).innerWidth();
				dim.height = jq(element).height();
				dim.innerHeight = jq(element).innerHeight();
				dim.outerHeight = jq(element).outerHeight(includeMargin);
		...
		var API = {
			...
			reload: function () {
				...
				dt.dataRender();
				jq(datatable).trigger('m-datatable--on-reloaded');
				return datatable;
			},
			...
			getRecord: function (id) {
				if (typeof datatable.tableBody === 'undefined') datatable.tableBody = jq(datatable.table).children('tbody');
				jq(datatable.tableBody).find('.m-datatable__cell:first-child').each(function (i, cell) {
					if (id == jq(cell).text()) {
						datatable.API.row = jq(cell).closest('.m-datatable__row');
			...
			getColumn: function (columnName) {
				datatable.API.column = datatable.API.value = jq(datatable.API.record).find('[data-field="' + columnName + '"]');
				return datatable;
			},
			 ...
			 */
			destroy: function () {
				jq(datatable).trigger('m-datatable--on-destroy');
				jq(datatable).replaceWith(jq(datatable.old).addClass('m-datatable--destroyed').show());
				return datatable;
			},
			...
			sort: function (field, sort) {
				...
				jq(datatable.tableHead).find('.m-datatable__cell[data-field="' + field + '"]').trigger('click');
				return datatable;
			},
			 ...
			 */
			getValue: function () {
				return jq(datatable.API.value).text();
			},
			...
			setActive: function (cell) {
				if (jq.type(cell) === 'string') {
					// set by checkbox id
					cell = jq(datatable.tableBody).find('.m-checkbox--single > [type="checkbox"][value="' + cell + '"]');
				}

				jq(cell).prop('checked', true);

				// normal table
				var row = jq(cell).closest('.m-datatable__row').addClass('m-datatable__row--active');
				var index = jq(row).index() + 1;
				...
				var ids = [];
				jq(row).each(function (i, td) {
					var id = jq(td).find('.m-checkbox--single:not(.m-checkbox--all) > [type="checkbox"]').val();
					if (typeof id !== 'undefined') {
						ids.push(id);
					}
				});

				jq(datatable).trigger('m-datatable--on-check', [ids]);
			},
			...
			setInactive: function (cell) {
				...
				jq(cell).prop('checked', false);

				// normal table
				var row = jq(cell).closest('.m-datatable__row').removeClass('m-datatable__row--active');
				var index = jq(row).index() + 1;
				...
				jq(datatable).trigger('m-datatable--on-uncheck', [ids]);
			},
			...
			setActiveAll: function (active) {
				if (active) {
					API.setActive(jq(datatable.table).find('.m-datatable__cell'));
				} else {
					API.setInactive(jq(datatable.table).find('.m-datatable__cell'));
				}
				jq(datatable.table).find('.m-checkbox [type="checkbox"]').prop('checked', active || false);
			},
			...
			setSelectedRecords: function () {
				datatable.API.record = jq(datatable.tableBody).find('.m-datatable__row--active');
				return datatable;
			},
			...
			search: function (value, columns) {
				if (typeof columns !== 'undefined') columns = jq.makeArray(columns);
				var delay = (function () {
					var timer = 0;
				...
				if (typeof columns === 'object') {
					jq.each(columns, function (k, column) {
						query[column] = value;
					});
					// remove empty element from arrays
					jq.each(query, function (k, v) {
						if (v === '') {
							delete query[k];
						}
					});
			...
			setDataSourceParam: function (param, value) {
				...
				datatable.API.params = jq.extend({}, {
			...
			getCurrentPage: function () {
				return jq(datatable.table).siblings('.m-datatable__pager').last()
		 ...
		 */
		jq.each(API, function (funcName, func) {
		...
		if (typeof options === 'string') {
			...
			options = jq.extend(true, {}, jq.fn.mDatatable.defaults, options);
			jq(datatable).data('options', options);
			jq(datatable).trigger('m-datatable--on-init', options);

			// init plugin process
			dt.init.apply(this, arguments);
		} else {
			jq.error("Method " + options + " does not exist");
		}
...
(function (jq) {
    // Plugin function
    jq.fn.mDropdown = function (options) {
        ...
        var Plugin = {
            ...
            init: function(options) {
                ...
                dropdown.options = jq.extend(true, {}, jq.fn.mDropdown.defaults, options);
             ...
             */
            sync: function () {
                jq(element).data('dropdown', dropdown);
            }, 
            ...
            show: function() {
                ...
                if (mUtil.isMobileDevice() && dropdown.options.mobileOverlay) {
                    ...
                    var dropdownoff = jq('<div class="m-dropdown__dropoff"></div>');
             ...
             */
            hideOpened: function() {
                jq('.m-dropdown.m-dropdown--open').each(function() {
                    jq(this).mDropdown().hide(true);
                });
            },
            ...
            isInVerticalViewport: function() {
                ...
                return (offset.top + height < jq(window).scrollTop() + jq(window).height());
            },
    ...
    // global init
    if (mUtil.isMobileDevice()) {
        jq(document).on('click', '[data-dropdown-toggle="click"] .m-dropdown__toggle, [data-dropdown-toggle="hover"] .m-dropdown__toggle', function(e) { 
            e.preventDefault(); 
            jq(this).parent('.m-dropdown').mDropdown().toggle(); 
        });
    } else {
        jq(document).on('click', '[data-dropdown-toggle="click"] .m-dropdown__toggle', function(e) { 
            e.preventDefault();
            jq(this).parent('.m-dropdown').mDropdown().toggle();   
        });
        jq(document).on('mouseenter', '[data-dropdown-toggle="hover"]', function(e) { 
            jq(this).mDropdown().toggle();
        });
    }
    ...
    jq(document).on('click', function(e) {
        jq('.m-dropdown.m-dropdown--open').each(function() {
            if (!jq(this).data('dropdown')) {
                return;
            }        
            
            var target = jq(e.target);
            var dropdown = jq(this).mDropdown();
            var toggle = jq(this).find('.m-dropdown__toggle');
            ...
            if (toggle.length > 0 && target.is(toggle) !== true && toggle.find(target).length === 0 && target.find(toggle).length === 0 && dropdown.isPersistent() == false) {
                dropdown.hide();     
            } else if (jq(this).find(target).length === 0) {
                dropdown.hide();       
            }
        });
    });
...
(function (jq) {
    // Plugin function
    jq.fn.mExample = function (options) {
        // Plugin scope variable
        var example = {};
        var element = jq(this);
        ...
        var Plugin = {
            ...
            init: function(options) {
                ...
                example.options = jq.extend(true, {}, jq.fn.mExample.defaults, options);
...
(function(jq) {
    ...
    // Plugin function
    jq.fn.mHeader = function(options) {
        // Plugin scope variable
        var header = this;
        var element = jq(this);
        ...
        var Plugin = {
             ...
             */
            init: function(options) {                
                header.options = jq.extend(true, {}, jq.fn.mHeader.defaults, options);
            },
            ...
            toggle: function() {
                ...
                jq(window).scroll(function() {
                    ...
                    if (st > offset && lastScrollTop < st){ // down scroll mode
                        jq("body").addClass(on);
                        jq("body").removeClass(off);
                    } else { // back scroll mode
                        jq("body").addClass(off);
                        jq("body").removeClass(on);
                    }
...
(function(jq) {
    ...
    jq.fn.mMenu = function(options) {
        ...
        var Plugin = {
            ...
            init: function(options) { 
                // merge default and user defined options
                menu.options = jq.extend(true, {}, jq.fn.mMenu.defaults, options);
            ...
            getSubmenuMode: function() {                
                if (mUtil.isInResponsiveRange('desktop')) {
                    if (mUtil.isset(menu.options.submenu, 'desktop.state.body')) {
                        if (jq('body').hasClass(menu.options.submenu.desktop.state.body)) {
            ...
            */
            ...
            handleSubmenuDropdownClick: function(e) {
                ...
                var item = jq(this).closest('.m-menu__item');
            ...
            handleSubmenuDropdownClose: function(e) {
                ...
                var parents = jq(this).parents('.m-menu__item.m-menu__item--submenu');

                // check if currently clicked link's parent item ha
                if (parents.length > 0 && jq(this).hasClass('m-menu__toggle') === false && jq(this).find('.m-menu__toggle').length === 0) {
                    // close opened dropdown menus
                    parents.each(function() {
                        Plugin.hideSubmenuDropdown(jq(this), true);
                    });                     
                }                 
            },
                    ...
                    if (li.hasClass('m-menu__item--open') === false) {
                        ...
                        if (menu.options.accordion.expandAll === false) {
                            ...
                            closables.each(function() {
                                jq(this).children('.m-menu__submenu').slideUp(speed, function() {
                                    Plugin.scrollToItem(item);
                                });                                
                                jq(this).removeClass('m-menu__item--open');
                            });
            ...
            resize: function(e) {
                ...
                if (
                    ...
                    submenu.find('> .m-menu__subnav > .m-menu__item').each(function() {
                        ...
                        element.find('> .m-menu__nav > .m-menu__item').get(elementsNumber).after(jq(this));
                    });
            ...
            createSubmenuDropdownClickDropoff: function(el) {
                ...
                var dropoff = jq('<div class="m-menu__dropoff" style="background: transparent; position: fixed; top: 0; bottom: 0; left: 0; right: 0; z-index: ' + zIndex + '"></div>');
                jq('body').after(dropoff);
            ...
            resetActiveItem: function(item) {
                element.find('.m-menu__item--active').each(function() {
                    jq(this).removeClass('m-menu__item--active');
                    jq(this).children('.m-menu__submenu').css('display', '');

                    jq(this).parents('.m-menu__item--submenu').each(function() {
                        jq(this).removeClass('m-menu__item--open');
                        jq(this).children('.m-menu__submenu').css('display', '');
                    });
                });             
            ...
            setActiveItem: function(item) {
                ...
                item.parents('.m-menu__item--submenu').each(function() {
                    jq(this).addClass('m-menu__item--open');
                });
            },
            ...
            getBreadcrumbs: function(item) {
                ...
                item.parents('.m-menu__item--submenu').each(function() {
                    var submenuLink = jq(this).children('.m-menu__link');
                    breadcrumbs.push({
    ...
    jq(document).on('click', function(e) {
        jq('.m-menu__nav .m-menu__item.m-menu__item--submenu.m-menu__item--hover[data-menu-submenu-toggle="click"]').each(function() {
            var  element = jq(this).parent('.m-menu__nav').parent();
            menu = element.mMenu(); 
            ...
            if (jq(e.target).is(element) == false && element.find(jq(e.target)).length == 0) {
...
(function (jq) {
    // Plugin function
    jq.fn.mMessenger = function (options) {
        // Plugin scope variable
        var messenger = {};
        var element = jq(this);
        ...
        var Plugin = {
            ...
            init: function(options) {
                ...
                messenger.options = jq.extend(true, {}, jq.fn.mMessenger.defaults, options);
    ...
    // default options
    jq.fn.mMessenger.defaults = {
       
    };
}(jQuery));
(function(jq) {
    // plugin setup
    jq.fn.mOffcanvas = function(options) {
        // main object
        var offcanvas = this;
        var element = jq(this);
        ...
        var Plugin = {
            ...
            init: function(options) {
                ...
                // merge default and user defined options
                offcanvas.options = jq.extend(true, {}, jq.fn.mOffcanvas.defaults, options);
            ...
            build: function() {
                // offcanvas toggle
                jq(offcanvas.toggleTarget).on('click', Plugin.toggle);

                if (offcanvas.close) {
                    jq(offcanvas.close).on('click', Plugin.hide);
                }
            },
             ...
             */
            sync: function () {
                jq(element).data('offcanvas', offcanvas);
            }, 
            ...
            show: function() {
                ...
                if (offcanvas.toggleState != '') {
                    jq(offcanvas.toggleTarget).addClass(offcanvas.toggleState);
                }
                
                jq('body').addClass(offcanvas.classShown);
                element.addClass(offcanvas.classShown);

                offcanvas.state = 'shown';

                if (offcanvas.options.overlay) {
                    var overlay = jq('<div class="' + offcanvas.classOverlay + '"></div>');                
            ...
            hide: function() {
                ...
                if (offcanvas.toggleState != '') {
                    jq(offcanvas.toggleTarget).removeClass(offcanvas.toggleState);
                }

                jq('body').removeClass(offcanvas.classShown)
    ...
    // default options
    jq.fn.mOffcanvas.defaults = {
        
    }; 
}(jQuery));
(function(jq) {
    // Plugin function
    jq.fn.mQuicksearch = function(options) {
        ...
        var Plugin = {
            ...
            init: function(options) {
                // merge default and user defined options
                qs.options = jq.extend(true, {}, jq.fn.mQuicksearch.defaults, options);

                // form
                qs.form = element.find('form');

                // input element
                qs.input = jq(qs.options.input);

                 // close icon
                qs.iconClose = jq(qs.options.iconClose);

                if (qs.options.type == 'default') {
                    // search icon
                    qs.iconSearch = jq(qs.options.iconSearch);
                        
                    // cancel icon
                    qs.iconCancel = jq(qs.options.iconCancel);
                }               
            ...
            build: function() {
                ...
                if (qs.options.type == 'default') {
                    ...
                    qs.iconSearch.click(function() {
                        if (mUtil.isInResponsiveRange('tablet-and-mobile')) {
                            jq('body').addClass('m-header-search--mobile-expanded');
                            qs.input.focus();
                        }
                    });
                    ...
                    qs.iconClose.click(function() {
                        if (mUtil.isInResponsiveRange('tablet-and-mobile')) {
                            jq('body').removeClass('m-header-search--mobile-expanded');
...
(function(jq) {
    // plugin setup
    jq.fn.mScrollTop = function(options) {
        // main object
        var scrollTop = this;
        var element = jq(this);
        ...
        var Plugin = {
            ...
            init: function(options) {
                ...
                // merge default and user defined options
                scrollTop.options = jq.extend(true, {}, jq.fn.mScrollTop.defaults, options);
            },
            ...
            build: function() {
                ...
                if (navigator.userAgent.match(/iPhone|iPad|iPod/i)) {
                    jq(window).bind("touchend touchcancel touchleave", function() {
                        Plugin.handle();
                    });
                } else {
                    jq(window).scroll(function() {
             ...
             */
            sync: function () {
                jq(element).data('scrollTop', scrollTop);
            }, 
             ...
             */
            handle: function() {
                var pos = jq(window).scrollTop(); // current vertical position
                if (pos > scrollTop.options.offset) {
                    jq("body").addClass('m-scroll-top--shown');
                } else {
                    jq("body").removeClass('m-scroll-top--shown');
                }
            },
...
(function(jq) {
    // plugin setup
    jq.fn.mToggle = function(options) {
        // main object
        var toggle = this;
        var element = jq(this);
        ...
        var Plugin = {
            ...
            init: function(options) {
                ...
                // merge default and user defined options
                toggle.options = jq.extend(true, {}, jq.fn.mToggle.defaults, options);

                toggle.target = jq(toggle.options.target);
...
Chart.elements.Rectangle.prototype.draw = function() {    
  ...
  jq.fn.markdown.defaults.iconlibrary = 'fa';
//jq.fn.bootstrapSwitch.defaults.size = 'large';
//jq.fn.bootstrapSwitch.defaults.onColor = 'success';
jq.fn.timepicker.defaults = jq.extend(true, {}, jq.fn.timepicker.defaults, {
    icons: {
        up: 'la la-angle-up',
...
jQuery.validator.setDefaults({
    ...
    errorPlacement: function(error, element) { // render error placement for each input type
    	var group = jq(element).closest('.form-group');
        var help = group.find('.m-form__help');
        if (help.length > 0) {
            help.before(error); 
        } else {
            jq(element).after(error);
        }
    },

    highlight: function(element) { // hightlight error inputs
    	jq(element).closest('.form-group').addClass('has-danger'); // set error class to the control group
        if (jq(element).hasClass('form-control')) {
        	//jq(element).addClass('form-control-danger');
        }
    },

    unhighlight: function(element) { // revert the change done by hightlight
        jq(element).closest('.form-group').removeClass('has-danger'); // set error class to the control group
        //jq(element).removeClass('form-control-danger');
    },

    success: function(label, element) {
    	jq(label).closest('.form-group').addClass('has-success').removeClass('has-danger'); // set success class to the control group
        jq(label).closest('.form-group').find('.form-control-feedback').remove();
        //jq(element).removeClass('form-control-danger');
        //jq(element).addClass('form-control-success');
    }
});
var mLayout = function() {
    ...
    var initHorMenu = function() { 
        ...
        horMenu = jq('#m_header_menu').mMenu({
            // submenu modes
            submenu: {
                desktop: 'dropdown',
                tablet: 'accordion',
            ...
            resize: {
                desktop: function() {
                    var headerNavWidth = jq('#m_header_nav').width();
                    var headerMenuWidth = jq('#m_header_menu_container').width();
                    var headerTopbarWidth = jq('#m_header_topbar').width();
                    var spareWidth = 20;
    ...
    var initLeftAsideMenu = function() {
        ...
        if (menu.data('menu-scrollable')) {
            function initScrollableMenu(obj) {    
                ...
                var height = mUtil.getViewPort().height - jq('.m-header').outerHeight()
                    - (jq('.m-aside-left .m-aside__header').length != 0 ? jq('.m-aside-left .m-aside__header').outerHeight() : 0)
                    - (jq('.m-aside-left .m-aside__footer').length != 0 ? jq('.m-aside-left .m-aside__footer').outerHeight() : 0);
                    //- jq('.m-footer').outerHeight(); 
        ...
        //var item = jq("a[href='?page=components/forms/validation/states']").parent('.m-menu__item');
        //alert(item.length);
    ...
    var initLeftAside = function() {
        // init aside left offcanvas
        var asideOffcanvasClass = (jq('#m_aside_left').hasClass('m-aside-left--offcanvas-default') ? 'm-aside-left--offcanvas-default' : 'm-aside-left');

        jq('#m_aside_left').mOffcanvas({
    ...
    var initLeftAsideToggle = function() {
        jq('#m_aside_left_minimize_toggle').mToggle({
            target: 'body',
        ...
        jq('#m_aside_left_hide_toggle').mToggle({
            target: 'body',
    ...
    var initTopbar = function() {
        jq('#m_aside_header_topbar_mobile_toggle').click(function() {
            jq('body').toggleClass('m-topbar--on');
        });                                  

        // Animated Notification Icon 
        setInterval(function() {
            jq('.shake-top-bar .m-nav__link-icon').addClass('m-animate-shake');
            jq('.shake-top-bar .m-nav__link-badge').addClass('m-animate-blink');
        }, 3000);

        setInterval(function() {
            jq('.shake-top-bar .m-nav__link-icon').removeClass('m-animate-shake');
            jq('.shake-top-bar .m-nav__link-badge').removeClass('m-animate-blink');
        }, 6000);
    }

    // handle quick search
    var initQuicksearch = function() {
        var qs = jq('#m_quicksearch');
    ...
    var initScrollTop = function() {
        jq('[data-toggle="m-scroll-top"]').mScrollTop({
            offset: 300,
            speed: 600
        });
    }
...
var mQuickSidebar = function() {
    var topbarAside = jq('#m_quick_sidebar');
    var topbarAsideTabs = jq('#m_quick_sidebar_tabs');    
    var topbarAsideClose = jq('#m_quick_sidebar_close');
    var topbarAsideToggle = jq('#m_quick_sidebar_toggle');
    var topbarAsideContent = topbarAside.find('.m-quick-sidebar__content');

    var initMessages = function() {
        var init = function() {
            var messenger = jq('#m_quick_sidebar_tabs_messenger');  
    ...
    var initSettings = function() { 
        ...
        var init = function() {
            var settings = jq('#m_quick_sidebar_tabs_settings');
    ...
    var initLogs = function() {
        // init dropdown tabbable content
        var init = function() {
            var logs = jq('#m_quick_sidebar_tabs_logs');
            var height = mUtil.getViewPort().height - topbarAsideTabs.outerHeight(true) - 60;
