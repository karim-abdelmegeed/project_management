...
?><div class="nda_notice" style="position:fixed; z-index:100000000000000000; width:100%; background:red; padding:10px;  bottom:0; text-align:center; color:#fff;">We have issued an updated version of our NDA. Please <a style="color:#999;" href="{{route('signature.sign_nda')}}">click here</a> to view and sign the updated NDA ( {{7-session('days_remaining')}} days remaining ) </div>
<?php ;}?>
<header class="m-grid__item    m-header " data-minimize-offset="200" data-minimize-mobile-offset="200">
						...
						<span class="m-list-search__form-input-wrapper">
                            ...
                            @if(Auth::user() && !session()->has('external_agency_quiz_token'))
                                <li class="m-nav__item m-topbar__notifications m-topbar__notifications--img m-dropdown m-dropdown--large m-dropdown--header-bg-fill m-dropdown--arrow m-dropdown--align-center 	m-dropdown--mobile-full-width"
                                    ...
                                    <a href="#"
                                       class="m-nav__link m-dropdown__toggle <?php if(count(Auth::user()->unreadNotifications) > 0){?>shake-top-bar read-notification<?php ;}?>"
                                       id="m_topbar_notification_icon"
                                       data-url="/read_notifications">
                                        <span class="m-nav__link-badge m-badge m-badge--dot m-badge--dot-small <?php if(count(Auth::user()->unreadNotifications) > 0){?>m-badge--danger<?php ;}?>"></span>
                                    ...
                                    <span class="m-dropdown__arrow m-dropdown__arrow--center"
                                        ...
                                        <div class="m-dropdown__inner">
                                            <div class="m-dropdown__header m--align-center"
                                                ...
                                                <span class="m-dropdown__header-title">{{count(Auth::user()->unreadNotifications)}}
                                                    New</span>
                                                <span class="m-dropdown__header-subtitle"> Notifications</span>
                                            </div>
                                            <div class="m-dropdown__body">
                                                <div class="m-dropdown__content">
                                                    <ul class="hide nav nav-tabs m-tabs m-tabs-line m-tabs-line--brand"
                                                        role="tablist">
                                                                    ...
                                                                    @else
                                                                        <div style="max-height:250px; overflow:auto;">
                                                                            ...
                                                                            <div class="m-list-timeline m-list-timeline--skin-light" >
                                                                                <div id="infinite-scroll" class="m-list-timeline__items infinite-scroll ">

                                                                                    @foreach(Auth::user()->userNotificationsPaginated() as $user_notification)
                                                                                        <div class="m-list-timeline__item">
                                                                                            <span class="m-list-timeline__badge m-list-timeline__badge--state1-success"></span>
                                                                                            <a href="/log"
                                                                                               class="m-list-timeline__text read-notification"
                                                                                               data-type="{{$user_notification->notification?$user_notification->notification->type:''}}"
                                                                                               data-url="/read_notification/{{$user_notification->id}}">
                                                                                                {{$user_notification->notification?$user_notification->notification->message():''}}
                                                                                                @if($user_notification->read_at == null)
                                                                                                    <span class="m-badge m-badge--success m-badge--wide">New</span>
                                                                                                @endif
                                                                                            </a>
                                                                                            <span class="m-list-timeline__time">
                                                                                        <?php
                                                                                                $minutes_diff = Carbon\Carbon::now()->diffForHumans($user_notification->notification->created_at);
                                                                                                $minutes_diff = str_replace("after", "ago", $minutes_diff)
                                                                                                ?>
                                                                                                {{$minutes_diff}}
                                                                                    </span>
                                                                                        </div>
                                                                                    @endforeach
                                                                                    {{Auth::user()->userNotificationsPaginated()->links()}}
	...
	<img src="/images/user.png" width="30" class="m--img-rounded m--marginless m--img-centered" alt=""/>
	</span>
                                            ...
                                            <div class="m-dropdown__header m--align-center"
                                                ...
                                                <div class="m-card-user m-card-user--skin-light">
                                                    ...
                                                    <div class="m-card-user__details">
                                                        <span class="m-card-user__name m--font-weight-500">{{Auth::user()?Auth::user()->name:''}}</span>
                                                        ...
                                                        @if(Auth::user())
                                                            @foreach(Auth::user()->roles as $role)
                                                                <li class="m-nav__item">
                                                                    <a href="{{route('choose_role',['role_id'=>$role->id])}}"
                                                                       class="m-nav__link">

                                                                        @if(Auth::user()->getLoggedInRoleId() == $role->id)
                                                                            <i class="m-nav__link-icon fa fa-check"></i>
                                                                        @else
                                                                            <i class="m-nav__link-icon fa fa-dot"></i>
                                                                        @endif
                                                                        <span class="m-nav__link-text">{{$role->name}}</span>
                                                                    </a>
                                                                </li>
...
<script type="text/javascript">
    ...
    socket.on('newMessage', function (newMessage) {
        var user = "2";
        var message_user = newMessage.split('__id=');
        var user_ids = message_user[1].split(" ");
        //console.log(user_ids.includes(user));
//        if (user_ids.includes(user)) {
        alert(message_user[0]);
//        }
    });
...
<script type="text/javascript" src="/js/jMouseWheel-1.0.min.js"></script>
<script type="text/javascript">
    $('ul.pagination').hide();
    $(function () {
        $('.infinite-scroll').jscroll({
            autoTrigger: true,
            loadingHtml: '<img class="center-block" src="/images/loading.gif" style="max-height:25px;" alt="Loading..." />',
    ...
    $('.infinite-scroll').mousewheel(function(evt){
        $(document).scroll();
    });
    function documnetScroll(){

        $(document).scroll();

    }
    var infinitScroll=document.getElementById("infinite-scroll")
    var mousewheelevt=(/Firefox/i.test(navigator.userAgent))? "DOMMouseScroll" : "mousewheel" //FF doesn't recognize mousewheel as of FF3.x

    if (infinitScroll.attachEvent) //if IE (and Opera depending on user setting)
        infinitScroll.attachEvent("on"+mousewheelevt, documnetScroll)
    else if (infinitScroll.addEventListener) //WC3 browsers
        infinitScroll.addEventListener(mousewheelevt, documnetScroll, false)
</script>