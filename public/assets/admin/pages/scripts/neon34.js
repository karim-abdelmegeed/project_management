$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon34/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_424]')
    ...
    $("select[name=user_name_424]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_424]").val();
       if(values===null){
           contacts_424.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_424.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_424.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_424 = $('#contacts-424').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-424').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-424').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_424.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-424search-form', function () {
        contacts_424.draw();
    });
    $(".filter").change(function () {
        contacts_424.search('');
        $(this).each(function () {
            contacts_424.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-424']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_424.row($row).remove().draw();
            html2canvas($("#contacts-424"), {
