$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon13/assignment_coordinator_id_data',function(response){
        response.forEach(function(element){
            $('select[name=assignment_coordinator_id_386]')
    ...
    $(document).on("click", ".multiple_delete", function () {
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon13/deleteCells", {"item": deleted_items}, function (response) {
                toastr.success(response.delete)
            });
            neon6_386.clear().draw()
        }
        else {
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon13/deleteCells", {"item": $(this).val()}, function (response) {
                        neon6_386.rows($item.closest('tr')).remove().draw();
                        toastr.success(response.delete)
                    });
                }
            });
     ...
     if ($('.reorder').length) {
            var drake = dragula([$('#neon6-386 tbody')[0]]);
    ...
    $('#neon6-386search-form').on('click', function (e) {
    ...
    var neon6_386 = $('#neon6-386').DataTable({
    ...
    neon6_386.column( 0 ).visible( false );
    $(".num_range").slider({
        range: true,
        min: 0,
        max: 1000,
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon6_386.row($row).remove().draw();
            html2canvas($("#neon6-386"), {
