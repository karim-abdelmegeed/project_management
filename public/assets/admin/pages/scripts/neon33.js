$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon33/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_422]')
    ...
    $("select[name=user_name_422]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_422]").val();
       if(values===null){
           contacts_422.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_422.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_422.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_422 = $('#contacts-422').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-422').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-422').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_422.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-422search-form', function () {
        contacts_422.draw();
    });
    $(".filter").change(function () {
        contacts_422.search('');
        $(this).each(function () {
            contacts_422.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-422']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_422.row($row).remove().draw();
            html2canvas($("#contacts-422"), {
