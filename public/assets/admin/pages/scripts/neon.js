$(document).on('ready pjax:success' , function () {
    $("select.select2").select2();
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon/deleteCells", {"item": deleted_items}, function (response) {
            ...
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon/deleteCells", {"item": $(this).val()}, function (response) {
    ...
    var neon_364 = $('#neon-364').DataTable({

     "initComplete": function (settings, json) {
            $('#neon-364').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            //--@Advanced-search@--
        },
    ...
    $(document).on('click', '#neon-364search-form', function () {
        neon_364.draw();
    });
    $(".filter").change(function () {
        neon_364.search('');
        $(this).each(function () {
            neon_364.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['neon-364']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon_364.row($row).remove().draw();
            html2canvas($("#neon-364"), {
