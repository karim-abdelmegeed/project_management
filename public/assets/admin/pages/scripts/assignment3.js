$(document).on('ready pjax:success' , function () {
    $("select.select2").select2();
     ...
     if ($('.reorder').length) {
            var drake = dragula([$('#assignment-360 tbody')[0]]);
    ...
    $('#assignment-360search-form').on('click', function (e) {
    ...
    var assignment_360 = $('#assignment-360').DataTable({
        ...
        columns: column_data,
        ajax: {
            url: '/assignment3/assignment3_data',
            data: function (d) {
                $('.element').each(function () {
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
           ...
           assignment_360.row($row).remove().draw();
            html2canvas($("#assignment-360"), {
