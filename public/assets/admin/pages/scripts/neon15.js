$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon15/assignment_coordinator_id_data',function(response){
        response.forEach(function(element){
            $('select[name=assignment_coordinator_id_390]')
    ...
    $(document).on("click", ".multiple_delete", function () {
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon15/deleteCells", {"item": deleted_items}, function (response) {
                toastr.success(response.delete)
            });
            neon6_390.clear().draw()
        }
        else {
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon15/deleteCells", {"item": $(this).val()}, function (response) {
                        neon6_390.rows($item.closest('tr')).remove().draw();
                        toastr.success(response.delete)
                    });
                }
            });
     ...
     if ($('.reorder').length) {
            var drake = dragula([$('#neon6-390 tbody')[0]]);
    ...
    $('#neon6-390search-form').on('click', function (e) {
    ...
    var neon6_390 = $('#neon6-390').DataTable({
    ...
    neon6_390.column( 0 ).visible( false );
    $(".num_range").slider({
        range: true,
        min: 0,
        max: 1000,
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon6_390.row($row).remove().draw();
            html2canvas($("#neon6-390"), {
