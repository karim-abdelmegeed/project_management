$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon26/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_408]')
    ...
    $("select[name=user_name_408]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_408]").val();
       if(values===null){
           contacts_408.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_408.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_408.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_408 = $('#contacts-408').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-408').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-408').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_408.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-408search-form', function () {
        contacts_408.draw();
    });
    $(".filter").change(function () {
        contacts_408.search('');
        $(this).each(function () {
            contacts_408.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-408']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_408.row($row).remove().draw();
            html2canvas($("#contacts-408"), {
