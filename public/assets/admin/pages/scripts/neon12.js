$(document).on('ready pjax:success' , function () {
    ...
    $(document).on("click", ".multiple_delete", function () {
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon12/deleteCells", {"item": deleted_items}, function (response) {
                toastr.success(response.delete)
            });
            neon6_384.clear().draw()
        }
        else {
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon12/deleteCells", {"item": $(this).val()}, function (response) {
                        neon6_384.rows($item.closest('tr')).remove().draw();
                        toastr.success(response.delete)
                    });
                }
            });
     ...
     if ($('.reorder').length) {
            var drake = dragula([$('#neon6-384 tbody')[0]]);
    ...
    $('#neon6-384search-form').on('click', function (e) {
    ...
    var neon6_384 = $('#neon6-384').DataTable({
    ...
    neon6_384.column( 0 ).visible( false );
    $(".num_range").slider({
        range: true,
        min: 0,
        max: 1000,
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon6_384.row($row).remove().draw();
            html2canvas($("#neon6-384"), {
