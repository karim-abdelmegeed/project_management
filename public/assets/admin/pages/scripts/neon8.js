$(document).on('ready pjax:success' , function () {
    ...
    $(document).on("click", ".multiple_delete", function () {
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon8/deleteCells", {"item": deleted_items}, function (response) {
                toastr.success(response.delete)
            });
            neon6_376.clear().draw()
        }
        else {
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon8/deleteCells", {"item": $(this).val()}, function (response) {
                        neon6_376.rows($item.closest('tr')).remove().draw();
                        toastr.success(response.delete)
                    });
                }
            });
    ...
    var neon6_376 = $('#neon6-376').DataTable({
     "initComplete": function (settings, json) {
            $('#neon6-376').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            //--@Advanced-search@--
        },
    ...
    neon6_376.columns().search( '' ).draw();
    if ($('.reorder').length) {
        var drake = dragula([$('#neon6-376 tbody')[0]]);
        drake.on('drop', function (el, target, source, sibling) {
            $('.reorder').prop('disabled', false);
        });
    }

    $(document).on('click', '#neon6-376search-form', function () {
        neon6_376.draw();
    });
    $(".filter").change(function () {
        neon6_376.search('');
        $(this).each(function () {
            neon6_376.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
    ...
    var allowFilter = ['neon6-376']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon6_376.row($row).remove().draw();
            html2canvas($("#neon6-376"), {
