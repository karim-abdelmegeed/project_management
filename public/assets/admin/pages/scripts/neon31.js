$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon31/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_418]')
    ...
    $("select[name=user_name_418]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_418]").val();
       if(values===null){
           contacts_418.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_418.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_418.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_418 = $('#contacts-418').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-418').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-418').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_418.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-418search-form', function () {
        contacts_418.draw();
    });
    $(".filter").change(function () {
        contacts_418.search('');
        $(this).each(function () {
            contacts_418.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-418']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_418.row($row).remove().draw();
            html2canvas($("#contacts-418"), {
