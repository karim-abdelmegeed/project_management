$(document).on('ready pjax:success' , function () {
    $("select.select2").select2();
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon5/deleteCells", {"item": deleted_items}, function (response) {
                toastr.success(response.delete)
            });
            neon5_371.clear().draw()
        }
        else {
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon5/deleteCells", {"item": $(this).val()}, function (response) {
                        neon5_371.rows($item.closest('tr')).remove().draw();
                        toastr.success(response.delete)
                    });
                }
            });
    ...
    var neon5_371 = $('#neon5-371').DataTable({

     "initComplete": function (settings, json) {
            $('#neon5-371').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            //--@Advanced-search@--
        },
    ...
    $(document).on('click', '#neon5-371search-form', function () {
        neon5_371.draw();
    });
    $(".filter").change(function () {
        neon5_371.search('');
        $(this).each(function () {
            neon5_371.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['neon5-371']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon5_371.row($row).remove().draw();
            html2canvas($("#neon5-371"), {
