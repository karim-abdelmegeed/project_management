$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon30/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_416]')
    ...
    $("select[name=user_name_416]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_416]").val();
       if(values===null){
           contacts_416.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_416.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_416.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_416 = $('#contacts-416').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-416').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-416').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_416.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-416search-form', function () {
        contacts_416.draw();
    });
    $(".filter").change(function () {
        contacts_416.search('');
        $(this).each(function () {
            contacts_416.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-416']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_416.row($row).remove().draw();
            html2canvas($("#contacts-416"), {
