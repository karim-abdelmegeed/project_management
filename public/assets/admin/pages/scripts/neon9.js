$(document).on('ready pjax:success' , function () {
    ...
    $(document).on("click", ".multiple_delete", function () {
        ...
        if ($(".group-checkable").is(":checked")) {
            ...
            $.post("/neon9/deleteCells", {"item": deleted_items}, function (response) {
                toastr.success(response.delete)
            });
            neon6_378.clear().draw()
        }
        else {
            $("input[name^=items]").each(function () {
                if ($(this).is(":checked")) {
                    $item = $(this);
                    $.post("/neon9/deleteCells", {"item": $(this).val()}, function (response) {
                        neon6_378.rows($item.closest('tr')).remove().draw();
                        toastr.success(response.delete)
                    });
                }
            });
    ...
    var neon6_378 = $('#neon6-378').DataTable({
     "initComplete": function (settings, json) {
            $('#neon6-378').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            //--@Advanced-search@--
        },
    ...
    neon6_378.columns().search( '' ).draw();
    if ($('.reorder').length) {
        var drake = dragula([$('#neon6-378 tbody')[0]]);
        drake.on('drop', function (el, target, source, sibling) {
            $('.reorder').prop('disabled', false);
        });
    }

    $(document).on('click', '#neon6-378search-form', function () {
        neon6_378.draw();
    });
    $(".filter").change(function () {
        neon6_378.search('');
        $(this).each(function () {
            neon6_378.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
    ...
    var allowFilter = ['neon6-378']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            neon6_378.row($row).remove().draw();
            html2canvas($("#neon6-378"), {
