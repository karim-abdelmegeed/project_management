$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon23/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_402]')
    ...
    $("select[name=user_name_402]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_402]").val();
       if(values===null){
           contacts_402.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_402.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_402.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_402 = $('#contacts-402').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-402').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-402').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_402.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-402search-form', function () {
        contacts_402.draw();
    });
    $(".filter").change(function () {
        contacts_402.search('');
        $(this).each(function () {
            contacts_402.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-402']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_402.row($row).remove().draw();
            html2canvas($("#contacts-402"), {
