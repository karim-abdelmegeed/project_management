$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon22/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_400]')
    ...
    $("select[name=user_name_400]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_400]").val();
       if(values===null){
           contacts_400.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_400.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_400.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_400 = $('#contacts-400').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-400').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-400').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_400.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-400search-form', function () {
        contacts_400.draw();
    });
    $(".filter").change(function () {
        contacts_400.search('');
        $(this).each(function () {
            contacts_400.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-400']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_400.row($row).remove().draw();
            html2canvas($("#contacts-400"), {
