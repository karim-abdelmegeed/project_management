$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon29/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_414]')
    ...
    $("select[name=user_name_414]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_414]").val();
       if(values===null){
           contacts_414.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_414.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_414.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_414 = $('#contacts-414').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-414').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-414').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_414.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-414search-form', function () {
        contacts_414.draw();
    });
    $(".filter").change(function () {
        contacts_414.search('');
        $(this).each(function () {
            contacts_414.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-414']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_414.row($row).remove().draw();
            html2canvas($("#contacts-414"), {
