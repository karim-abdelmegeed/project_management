$(document).on('ready pjax:success' , function () {
    ...
    $.get('/neon24/user_name_data',function(response){
        response.forEach(function(element){
            $('select[name=user_name_404]')
    ...
    $("select[name=user_name_404]").change(function () {
        array_push=[];
       var values= $("select[name=user_name_404]").val();
       if(values===null){
           contacts_404.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
          ...
          $(this).each(function () {
          contacts_404.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
          contacts_404.column($(this).data('column-index')).search(array_push.join('|')).draw();
           });
       }
    });
    var contacts_404 = $('#contacts-404').DataTable({

     "initComplete": function (settings, json) {
            $('#contacts-404').closest('.m-portlet').find(".dataTables_filter").find('input').attr('placeholder', "Search...");
            $('#contacts-404').closest('.m-portlet').find(".dataTables_filter").find('input')
     ...
     //export-buttons
        ...
        contacts_404.columns().search( '' ).draw();
    ...
    $(document).on('click', '#contacts-404search-form', function () {
        contacts_404.draw();
    });
    $(".filter").change(function () {
        contacts_404.search('');
        $(this).each(function () {
            contacts_404.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });
          var ID;

    var allowFilter = ['contacts-404']; //useful for multiple tables in view
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            contacts_404.row($row).remove().draw();
            html2canvas($("#contacts-404"), {
