$(document).on('ready pjax:success' , function () {
    $("select.select2").select2();
        $.get('/freelancer_invoice/user_name_data',function(response){
    ...
    $.get('/freelancer_invoice/project_name_data',function(response){
        response.forEach(function(element){
    ...
    $("select[name=project_name]").change(function () {
        array_push=[];
       var values= $("select[name=project_name]").val();
       if(values===null){
           table.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
    ...
    $.get('/freelancer_invoice/subproject_project_id_data',function(response){
        response.forEach(function(element){
            $('.srch_form').find('select[name=subproject_project_id]')
                .append($("<option></option>")
                    .attr("value",element.project_id)
                    .text(element.project_id));
        });

    });
    $("select[name=subproject_project_id]").change(function () {
        array_push=[];
       var values= $("select[name=subproject_project_id]").val();
       if(values===null){
           table.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
       }
       else{
    ...
    var table = $('#invoice-256').DataTable({'scrollX':true,
"initComplete": function( settings, json ) {
  ...
  dom: 'Bfrtip',
    ...
    {
        "columnDefs": [
            {"searchable": false, "targets": [4,10,11,]}
        ]
