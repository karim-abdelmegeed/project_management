$(document).on('ready pjax:success' , function () {
    ...
    $.get('/search/role_name_data', function (response) {
     ...
     if ($('.reorder').length) {
            var drake = dragula([$('#n_d_a_history-246 tbody')[0]]);
    ...
    var table = $('#n_d_a_history-246').DataTable({'scrollX':true,
        "columnDefs": [
        ...
        columns: column_data,
        ajax: {
            url: '/nda_history/nda_history_data',
            data: function (d) {
                $('.element').each(function () {
