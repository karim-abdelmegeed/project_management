$(document).on('ready pjax:success', function () {
    var current_projects = $('#current_projects-261').DataTable({
            'scrollX': true,
            "initComplete": function (settings, json) {
                $('#current_projects-261').closest(".m-portlet").find(".dataTables_filter").find('input').attr('placeholder', "Search").after('&nbsp;&nbsp;<a href="#" class="advanced_search btn btn-secondary">Advanced Search</a>');
        ...
        {
            "columnDefs": [
                {"searchable": false, "targets": [13, 14, 23,]}
            ]
    ...
    $("select[name=current_service_name]").change(function () {
        ...
        if (values === null) {
            current_projects.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
        }
        else {
            ...
            $(this).each(function () {
                current_projects.column($(this).data('column-index')).search(array_push.join('|'), true, false, true).draw();
                current_projects.column($(this).data('column-index')).search(array_push.join('|')).draw();
            });
        }
    });
    ...
    current_projects.columns().search('').draw();
    ...
    $(document).on('click', '#search-form', function () {
        current_projects.draw();
    });
    $(".filter").change(function () {
        current_projects.search('');
        $(this).each(function () {
            current_projects.column($(this).data('column-index')).search($(this).val()).draw();
        });
    });

    var allowFilter = ['current_projects-261']; //useful for multiple tables in view
        ...
        swal({
            ...
            $.ajax({
                ...
                success: function () {
                    ...
                    current_projects.row($row).remove()
                        .draw();
                    html2canvas($("#current_projects-261"), {
