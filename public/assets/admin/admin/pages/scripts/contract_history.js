$(document).on('ready pjax:success' , function () {
    ...
    $.get('/search/project_name_data', function (response) {
     ...
     if ($('.reorder').length) {
            var drake = dragula([$('#contract_history-247 tbody')[0]]);
    ...
    var table = $('#contract_history-247').DataTable({'scrollX':true,
        "columnDefs": [
        ...
        columns: column_data,
        ajax: {
            url: '/contract_history/contract_history_data',
            data: function (d) {
                $('.element').each(function () {
...
//js-fn
    $(document).on('click', '.remove-stuff', function (e) {//Delete from list(rewrite the function in scripts.js)
        ...
        success: function () {
            ...
            .draw();
            html2canvas($("#contract_history-247"), {
